...
class BONECRAFT_PT_rename_bone(bpy.types.Panel):
    ...
    def draw_delete_button(self, box, target_parts):
        del_button = box.operator(
            "ezrenamer.rename_pose_bones", text="Delete", icon="CANCEL"
        )
    ...
    def draw_section(self, box, items, target_part):
        ...
        for i, item in enumerate(items):
            ops = col.operator(
                "ezrenamer.rename_pose_bones", text=item, translate=False
            )
...
panel_classes = [BONECRAFT_PT_rename_bone]
