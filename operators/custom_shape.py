...
def edit_custom_shape(self, context):
    ...
    mesh_obj.matrix_world = (
        armature.matrix_world @ base_matrix @ translation_matrix @ rotation_matrix
    )
        ...
        try:
            ...
            if current_obj != target_obj or current_mode != "EDIT_MESH":
                if current_obj.type == "MESH" and current_mode == "OBJECT":
                    ...
                    print(
                        f"Exceptional state: {current_obj.name} is in {current_mode} mode."
                    )
...
class BONECRAFT_OT_CreateCustomShape(bpy.types.Operator):
    ...
    delete_faces: bpy.props.BoolProperty(name="Delete Faces", default=True)
    set_cursor_to_bone_head: bpy.props.BoolProperty(
        name="Set Cursor to Bone Head", default=True
    )
