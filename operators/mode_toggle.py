...
class BONECRAFT_OT_mode_toggle(bpy.types.Operator):
    ...
    armature_mode: bpy.props.EnumProperty(
        name="Armature Mode",
        items=[("EDIT", "Edit", "Edit mode"), ("POSE", "Pose", "Pose mode")],
    )
    ...
    def switch_to_mesh_mode(self, context, reference_armature, target_mesh):
        ...
        MTU.switch_mode(context, target_mesh, self.rigged_mesh_mode, reference_armature)
        ...
        if active_bone_name and self.rigged_mesh_mode == "WEIGHT_PAINT":
            if vg := target_mesh.vertex_groups.get(active_bone_name, None):
                ...
                if self.add_new_vertex_group:
                    ...
                    self.report(
                        {"INFO"},
                        f"Active bone '{active_bone_name}' does not have a vertex group",
                    )
        return {"FINISHED"}
...
class BONECRAFT_OT_enter_weight_paint_mode(bpy.types.Operator):
    ...
    bl_description = (
        "Enter Weight Paint mode with the active bone's vertex group selected"
    )
    bl_options = {"REGISTER", "UNDO"}
...
operator_classes = [BONECRAFT_OT_mode_toggle, BONECRAFT_OT_enter_weight_paint_mode]
