...
class EZRENAMER_OT_CreateTestArmature(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # ボーンを10本追加
        for i in range(9):
            bpy.ops.armature.duplicate_move(TRANSFORM_OT_translate={"value": (0, 1, 0)})
...
class EZRENAMER_OT_RenamePoseBones(bpy.types.Operator, ArmModeMixin):
    ...
    def rename_selected_pose_bones(self, context):
        ...
        for i, rnb in enumerate(rn_bones, start=1):
            ...
            if "counter" in self.target_parts:
                ...
                if item:
                    if self.operation == "add/replace":
                        new_elements[self.target_parts] = item.get_value(
                            self.index + i
                        )  # FIXME: カウンターだけはインクリメントしないといけない
            ...
            self.DBG_RENAME and log.info(
                f"{i} / {len(rn_bones)}: {rnb.name} -> {rnb.new_name}"
            )
